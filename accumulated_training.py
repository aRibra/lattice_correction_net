...
def train_accumulated_datasets(
    X_train,
    y_train,
    X_val,
    y_val,
    ...
    - device: 'cpu' or 'cuda'
    - use_pinn: Enable physics-informed loss (PINN)
    - pinn_lambda: Weight for physics loss term
    ...
    for i in range(1, number_of_accumulated_datasets + 1):
        ...
        val_subset_dataset = TensorDataset(
            torch.tensor(X_val, dtype=torch.float32),
            torch.tensor(y_val, dtype=torch.float32),
        )
        val_subset_loader = DataLoader(
            val_subset_dataset, batch_size=batch_size, shuffle=False
        )
...
if __name__ == "__main__":
    ...
    train_inputs, val_inputs, train_targets, val_targets, data_shapes = get_data_splits(
        sim_data, test_size=0.10, model_arch=model_arch
    )
