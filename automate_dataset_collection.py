...
from synchrotron_simulator_gpu_Dataset_4D import (
    SynchrotronSimulator,
    SimulationRunner,
    QuadrupoleMisAlignError,
    QuadrupoleTiltError,
    DipoleTiltError,
)
...
class SimulationDataset:
    ...
    def process_simulated_data(
        self, start_rev, end_rev, fodo_cell_indices, planes, include_no_error_data=False
    ):
        """
        ...
        if self.merged_config["reject_multiple_error_types"] and len_defined_errors > 1:
            raise Exception(
                f"SimulationDataset/process_simulated_data(): Defining multiple error types is rejected in the config"
        ...
        for plane in planes:
            readings_plane_with_error = readings_with_error[plane][
                :, start_rev:end_rev, fodo_cell_indices
            ]
            # Compute mean over particles
            mean_with_error = np.mean(
                readings_plane_with_error, axis=0
            )  # Shape: [n_turns, n_BPMs]
            all_ravg_with_error = []

            # Loop over BPMs
            for cell_ix in range(len(fodo_cell_indices)):
                if self.apply_avg:
                    ravg_with_error = self.running_average_numpy(
                        mean_with_error[:, cell_ix],
            ...
            data_with_error.append(
                all_ravg_with_error[..., np.newaxis]
            )  # Shape: [n_turns, n_BPMs, 1]

            # Only process no_error data if it exists
            if readings_no_error is not None:
                readings_plane_no_error = readings_no_error[plane][
                    :, start_rev:end_rev, fodo_cell_indices
                ]
                mean_no_error = np.mean(
                    readings_plane_no_error, axis=0
                )  # Shape: [n_turns, n_BPMs]
                all_ravg_no_error = []

                for cell_ix in range(len(fodo_cell_indices)):
                    if self.apply_avg:
                        ravg_no_error = self.running_average_numpy(
                            mean_no_error[:, cell_ix],
                            window_size=end_rev - start_rev - 100,
                        )
                    else:
                ...
                data_no_error.append(
                    all_ravg_no_error[..., np.newaxis]
                )  # Shape: [n_turns, n_BPMs, 1]
        ...
        if not self.merged_config["target_data"]:
            target_error_values = []
            print(
                "[WARNING] SimulationDataset.process_simulated_data()/ `target_data` was not specified in the configuration. "
                "Therefore, there will be no errors applied."
            )
        elif (
            self.merged_config["target_data"] == "quad_misalign_deltas"
            and errors_state["quadrupole_errors"]
        ):
            target_error_values = error_values_quad_misalign
        elif (
            self.merged_config["target_data"] == "quad_tilt_angles"
            and errors_state["quadrupole_tilt_errors"]
        ):
            target_error_values = error_values_quad_tilt
        elif (
            self.merged_config["target_data"] == "dipole_tilt_angles"
            and errors_state["dipole_tilt_errors"]
        ):
            target_error_values = error_values_dipole_tilt
        else:
            raise ValueError(
                "This exception indicates that you selected a `target_data` type, "
        ...
        input_data_reshaped = input_data.reshape(
            n_training_samples * n_turns * n_BPMs, n_features
        )

        # Reshape back for torch
        input_data_reshaped = input_data_reshaped.reshape(
            n_training_samples, n_turns, n_BPMs, n_features
        )
        ...
        error_values_quad_misalign = torch.tensor(
            error_values_quad_misalign, dtype=torch.float32
        )[np.newaxis, ...]
        error_values_quad_tilt = torch.tensor(
            error_values_quad_tilt, dtype=torch.float32
        )[np.newaxis, ...]
        error_values_dipole_tilt = torch.tensor(
            error_values_dipole_tilt, dtype=torch.float32
        )[np.newaxis, ...]
...
class DataAutomation:
    def __init__(self, base_configurations, common_parameters, n_simulations):
        ...
        if quad_errors:
            ...
            fodo_indices_with_error_str = "".join(
                str(ei) for ei in fodo_indices_with_error
            )
        else:
        ...
        if quad_tilt_errors:
            fodo_indices_with_quad_tilt_error = [
                err["FODO_index"] for err in quad_tilt_errors
            ]
            fodo_indices_with_quad_tilt_error_str = "".join(
                str(ei) for ei in fodo_indices_with_quad_tilt_error
            )
        else:
            fodo_indices_with_quad_tilt_error = []
        ...
        if dipole_tilt_errors:
            fodo_indices_with_dipole_tilt_error = [
                err["FODO_index"] for err in dipole_tilt_errors
            ]
            fodo_indices_with_dipole_tilt_error_str = "".join(
                str(ei) for ei in fodo_indices_with_dipole_tilt_error
            )
        else:
            fodo_indices_with_dipole_tilt_error = []
    ...
    def compute_com_deltas(self, simulator_no_error, simulator_with_error, cell_idx=0):
        ...
        x_no_error = simulator_no_error.bpm_readings["x"][
            :, start_idx:end_idx, cell_idx
        ].mean()
        y_no_error = simulator_no_error.bpm_readings["y"][
            :, start_idx:end_idx, cell_idx
        ].mean()
        x_with_error = simulator_with_error.bpm_readings["x"][
            :, start_idx:end_idx, cell_idx
        ].mean()
        y_with_error = simulator_with_error.bpm_readings["y"][
            :, start_idx:end_idx, cell_idx
        ].mean()
    ...
    def run_simulations(
        self, skip_data_on_delta_ranges=False, include_no_error_data=False
    ):
        """
        ...
        include_no_error_data = self.common_parameters.get(
            "include_no_error_data", include_no_error_data
        )

        # Validate configuration parameters
        if (
            not self.common_parameters.get("delta_range")
            and not self.common_parameters.get("quad_tilt_angle_range")
            and not self.common_parameters.get("dipole_tilt_angle_range")
        ):
            raise ValueError(
                "No error ranges defined in configuration. Please specify at least one of: delta_range, quad_tilt_angle_range, or dipole_tilt_angle_range"
            )

        quad_misalign_delta_mean_or_min, quad_misalign_delta_std_or_max = (
            self.common_parameters.get("delta_range", (0, 0))
        )
        quad_tilt_mean_or_min, quad_tilt_std_or_max = self.common_parameters.get(
            "quad_tilt_angle_range", (0, 0)
        )
        dipole_tilt_mean_or_min, dipole_tilt_std_or_max = self.common_parameters.get(
            "dipole_tilt_angle_range", (0, 0)
        )
        delta_range_min, delta_range_max = self.common_parameters.get(
            "com_delta_range", (0, 0)
        )

        print(
            f"[run_simulations] Starting {self.n_simulations} simulations with include_no_error_data={include_no_error_data}"
        )
        print(
            f"[run_simulations] Error ranges: quad_misalign={quad_misalign_delta_mean_or_min, quad_misalign_delta_std_or_max}, "
        ...
        for sim_idx in tqdm(range(self.n_simulations), desc="Running Simulations"):
            ...
            if (
                ...
                and len_defined_errors > 1
            ):
                raise Exception(
                    "Defining multiple error types is not currently supported "
            ...
            if base_config["quad_tilt_errors"]:
                for quad_error_idx in range(len(base_config["quad_tilt_errors"])):
                    random_tilt_angle = sampling_func(
                        quad_tilt_mean_or_min, quad_tilt_std_or_max
                    )
                    base_config["quad_tilt_errors"][quad_error_idx]["tilt_angle"] = (
                        random_tilt_angle
                    )
            ...
            if base_config["dipole_tilt_errors"]:
                for quad_error_idx in range(len(base_config["dipole_tilt_errors"])):
                    random_tilt_angle = sampling_func(
                        dipole_tilt_mean_or_min, dipole_tilt_std_or_max
                    )
                    base_config["dipole_tilt_errors"][quad_error_idx]["tilt_angle"] = (
                        random_tilt_angle
                    )
            ...
            try:
                ...
                runner.run_configurations(
                    draw_plots=False,
                    verbose=False,
                    run_no_error_sim=include_no_error_data,
                )
                ...
                simulator_with_error = runner.simulators_with_error.get(
                    config_key_with_error
                )
                ...
                if simulator_with_error:
                    ...
                    if simulator_no_error is not None:
                        delta_x, delta_y = self.compute_com_deltas(
                            simulator_no_error, simulator_with_error
                        )
                    else:
                    ...
                    if (
                        skip_data_on_delta_ranges
                        and delta_x is not None
                        and delta_y is not None
                        and not (
                            delta_range_min <= delta_x <= delta_range_max
                            and delta_range_min <= delta_y <= delta_range_max
                        )
                    ):
                        print(
                            f"Simulation {sim_idx}: Delta out of range (delta_x={delta_x}, "
                        ...
                        bpm_readings_no_error = (
                            simulator_no_error.bpm_readings
                            if simulator_no_error is not None
                            else None
                        )
                        bpm_positions = (
                        ...
                        end_rev = self.common_parameters.get(
                            "end_rev", simulator_with_error.n_turns
                        )
                        ...
                        print(
                            f"After process_simulated_data()/ Input tensor shape: {input_tensor.shape}"
                        )

                        self.all_input_tensors.append(input_tensor)
                        self.all_target_tensors.append(target_tensor)
                        self.all_error_values_quad_misalign.append(
                            error_values_quad_misalign
                        )
                        self.all_error_values_quad_tilt.append(error_values_quad_tilt)
                        self.all_error_values_dipole_tilt.append(
                            error_values_dipole_tilt
                        )

                        k_delta = simulator_with_error._k_delta
                        k_noise_map = simulator_with_error._k_noise_map
                        if k_noise_map:
                            unique_fodo_indices = sorted(
                                set(k[0] for k in k_noise_map.keys())
                            )
                            k_noise_array = np.zeros(
                                (len(unique_fodo_indices), 2), dtype=np.float32
                            )
                            for fodo_idx in unique_fodo_indices:
                                quad_idx = unique_fodo_indices.index(fodo_idx)
                                for key, k_noise in k_noise_map.items():
                                    if key[0] == fodo_idx:
                                        if key[1] == "focusing":
                                            k_noise_array[quad_idx, 0] = k_noise
                                        else:
                                            k_noise_array[quad_idx, 1] = k_noise
                            self.all_k_noises.append(k_noise_array)
                        else:
                            self.all_k_noises.append(np.zeros((0, 2), dtype=np.float32))
                        self.all_k_deltas.append(k_delta)
                        ...
                        if self.common_parameters.get("verbose", False):
                            print(
                                f"[Memory Cleanup] Simulation {sim_idx} - RAM freed after data extraction."
                            )
    ...
    def run(self, skip_data_on_delta_ranges=False, include_no_error_data=False):
        ...
        self.run_simulations(
            skip_data_on_delta_ranges=skip_data_on_delta_ranges,
            include_no_error_data=include_no_error_data,
        )
    ...
    def save_data(self, postfix="accumulated"):
        ...
        if self.all_input_tensors and self.all_target_tensors:
            ...
            input_data_flat = self.all_input_tensors_torch.view(
                n_samples, -1
            )  # shape (n_samples, n_turns*n_BPMs*n_features)
            # Keep only rows where ALL features are finite (no NaNs, no Infs)
            ...
            if n_samples == 0:
                print(
                    "[WARNING] No samples available for scaling. Skipping scaling step."
                )
                ...
                input_data_reshaped = self.all_input_tensors_torch.reshape(
                    n_samples * n_turns * n_BPMs, n_features
                )
                input_data_scaled = self.data_scalers["input_scaler"].fit_transform(
                    input_data_reshaped
                )
                self.all_input_tensors_scaled_torch = input_data_scaled.reshape(
                    n_samples, n_turns, n_BPMs, n_features
                )
                self.all_input_tensors_scaled_torch = torch.tensor(
                    self.all_input_tensors_scaled_torch, dtype=torch.float32
                )

                if len(self.all_target_tensors_torch.shape) > 2:
                    n_samples, n_targets = self.all_target_tensors_torch.shape
                    target_data_reshaped = self.all_target_tensors_torch.reshape(
                        n_samples, n_targets
                    )
                else:
                    target_data_reshaped = self.all_target_tensors_torch

                self.all_target_tensors_scaled_torch = self.data_scalers[
                    "target_scaler"
                ].fit_transform(target_data_reshaped)
                self.all_target_tensors_scaled_torch = torch.tensor(
                    self.all_target_tensors_scaled_torch, dtype=torch.float32
                )

            self.all_error_values_quad_misalign_torch = torch.cat(
                self.all_error_values_quad_misalign, dim=0
            )
            self.all_error_values_quad_tilt_torch = torch.cat(
                self.all_error_values_quad_tilt, dim=0
            )
            self.all_error_values_dipole_tilt_torch = torch.cat(
                self.all_error_values_dipole_tilt, dim=0
            )

            max_n_quads = (
                max(kn.shape[0] for kn in self.all_k_noises) if self.all_k_noises else 0
            )
            k_deltas_tensor = torch.tensor(self.all_k_deltas, dtype=torch.float32)
            k_noises_padded = []
            for kn in self.all_k_noises:
                if kn.shape[0] < max_n_quads:
                    padding = np.zeros((max_n_quads - kn.shape[0], 2), dtype=np.float32)
                    kn_padded = np.vstack([kn, padding])
                    k_noises_padded.append(kn_padded)
                else:
                    k_noises_padded.append(kn)
            k_noises_tensor = torch.tensor(
                np.stack(k_noises_padded), dtype=torch.float32
            )
        else:
            ...
            k_deltas_tensor = None
            k_noises_tensor = None

        torch.save(self, f"{self.output_folder_path}/data_automation-{postfix}.pt")
        torch.save(
            self.merged_config, f"{self.output_folder_path}/merged_config-{postfix}.pt"
        )
        torch.save(
            self.all_input_tensors_torch,
            f"{self.output_folder_path}/input_tensors-{postfix}.pt",
        )
        torch.save(
            self.all_target_tensors_torch,
            f"{self.output_folder_path}/target_tensors-{postfix}.pt",
        )
        torch.save(
            self.all_input_tensors_scaled_torch,
            f"{self.output_folder_path}/input_tensors_scaled-{postfix}.pt",
        )
        torch.save(
            self.all_target_tensors_scaled_torch,
            f"{self.output_folder_path}/target_tensors_scaled-{postfix}.pt",
        )

        torch.save(
            self.all_error_values_quad_misalign_torch,
            f"{self.output_folder_path}/all_error_values_quad_misalign-{postfix}.pt",
        )
        torch.save(
            self.all_error_values_quad_tilt_torch,
            f"{self.output_folder_path}/all_error_values_quad_tilt-{postfix}.pt",
        )
        torch.save(
            self.all_error_values_dipole_tilt_torch,
            f"{self.output_folder_path}/all_error_values_dipole_tilt-{postfix}.pt",
        )

        torch.save(
            k_deltas_tensor,
            f"{self.output_folder_path}/all_k_deltas-{postfix}.pt",
        )
        torch.save(
            k_noises_tensor,
            f"{self.output_folder_path}/all_k_noises-{postfix}.pt",
        )

        torch.save(
            self.data_scalers, f"{self.output_folder_path}/dataset_scalers-{postfix}.pt"
        )
