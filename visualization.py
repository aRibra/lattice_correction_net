...
def plot_data_histograms(
    ...
    if merged_config["quad_errors"]:
        fodo_indices_with_error = [
            err["FODO_index"] for err in merged_config["quad_errors"]
        ]
    else:
        fodo_indices_with_error = []
    ...
    plt.grid()
    plt.savefig(
        f"{SAVE_DIR_FIGS}/delta_y_error_hist.eps", bbox_inches="tight", format="eps"
    )
    plt.show()
    ...
    if plot_com_deltas_x:
        plt.figure(figsize=(7, 6))
        plt.hist(
            np.array(data_automation.com_deltas_x) * 1e6,
            bins=30,
            edgecolor="black",
            color="skyblue",
        )
        ...
        plt.tight_layout()
        plt.savefig(
            f"{SAVE_DIR_FIGS}/com_deltas_x_hist.eps", bbox_inches="tight", format="eps"
        )
        plt.show()
    ...
    if plot_com_deltas_y:
        plt.figure(figsize=(7, 6))
        plt.hist(
            np.array(data_automation.com_deltas_y) * 1e6,
            bins=30,
            edgecolor="black",
            color="salmon",
        )
        ...
        plt.tight_layout()
        plt.savefig(
            f"{SAVE_DIR_FIGS}/com_deltas_y_hist.eps", bbox_inches="tight", format="eps"
        )
        plt.show()
    ...
    if plot_dipole_tilt_error_hist:
        ...
        legend_dipole_tilt = [
            f"dipole_{qix}" for qix in fodo_indices_with_dipole_tilt_error
        ]
        ...
        secax_dipole = plt.gca().secondary_xaxis(
            "top", functions=(mrad_to_deg, deg_to_mrad)
        )
        secax_dipole.set_xlabel("Tilt Angle (degrees)", fontsize=16)
        secax_dipole.tick_params(axis="x", labelsize=15)
        plt.tight_layout()
        plt.savefig(
            f"{SAVE_DIR_FIGS}/dipole_tilt_error_hist.eps",
            bbox_inches="tight",
            format="eps",
        )
        plt.show()
    ...
    if plot_quad_tilt_error_hist:
        ...
        secax_quad = plt.gca().secondary_xaxis(
            "top", functions=(mrad_to_deg, deg_to_mrad)
        )
        ...
        plt.tight_layout()
        plt.savefig(
            f"{SAVE_DIR_FIGS}/quad_tilt_error_hist.eps",
            bbox_inches="tight",
            format="eps",
        )
        plt.show()
...
def plot_benchmark_stats(stats, benchmark_info):
    ...
    for fodo_ix in fodo_indices:
        ...
        plt.plot(
            noise_pallette * 1e3
            if benchmark_type == "quad_tilt"
            else noise_pallette * 1e6,
            means * 1e6,
            "-" + marker,
            label=f"FODO-QE-ix {fodo_mapping[fodo_ix]}",
        )
    ...
    if benchmark_type == "bpm_shift":
        ...
        title = (
            f"Model Prediction Accuracy vs BPM Shift Level\n"
        ...
        title = (
            f"Model Prediction Accuracy vs Quad Tilt Noise Level (noise_range: {noise_range}, bins: {bins})\n"
        ...
        title = (
            f"Model Prediction Accuracy vs BPM Noise Level (noise_range: {noise_range}, bins: {bins})\n"
            f"MisAlign error: {not cancel_misalign_error}\nTilt errors: {not cancel_tilt_error}"
        )
        save_name = "plot_accuracy_benchmark_bpm_noise.eps"

    ylabel = (
        f"Mean Absolute Error (µm)\nAveraging over {runs_per_noise} simulation runs\n"
    )
    ...
    ax_main = plt.gca()
    ax_inset = inset_axes(
        ax_main, width="30%", height="10%", loc="lower right", borderpad=2.5
    )
    ...
    if noise_zero_key in mean_errors:
        mae_zero = [
            mean_errors[noise_zero_key].get(fodo_ix, 0) * 1e6
            for fodo_ix in fodo_indices
        ]
        ...
        ax_inset.set_title(
            f"MAE at Shift≈0" if benchmark_type == "bpm_shift" else "MAE at Noise=0",
            fontsize=11,
        )
...
def plot_training_results(num_epochs, train_losses, val_losses, train_maes, val_maes):
    ...
    plt.grid(True)
    plt.savefig(
        f"{SAVE_DIR_FIGS}/train_loss_loglog.eps", bbox_inches="tight", format="eps"
    )
    plt.show()
...
def plot_pinn_losses(
    ...
    """
    Plot the data loss and physics loss components separately for PINN training.

    Args:
        num_epochs (int): Number of training epochs.
        train_data_losses (list): List of training data (MSE) losses.
        train_phys_losses (list): List of training physics losses.
        val_data_losses (list): List of validation data (MSE) losses.
        val_phys_losses (list): List of validation physics losses.
        save_dir (str, optional): Directory to save the plots. If None, uses SAVE_DIR_FIGS.
    Returns:
        None
    """
    import os
    ...
    save_dir_figs = (
        SAVE_DIR_FIGS if save_dir is None else os.path.join(save_dir, "figs")
    )
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(epochs, train_data_losses, label="Train Data Loss (MSE)", color="blue")
    plt.plot(epochs, train_phys_losses, label="Train Physics Loss", color="orange")
    plt.xlabel("Epoch", fontsize=18)
    plt.ylabel("Loss", fontsize=18)
    plt.title("Training Data and Physics Losses", fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.savefig(
        f"{save_dir_figs}/train_pinn_losses.eps", bbox_inches="tight", format="eps"
    )
    plt.show()

    plt.figure(figsize=(10, 5))
    plt.plot(epochs, val_data_losses, label="Val Data Loss (MSE)", color="blue")
    plt.plot(epochs, val_phys_losses, label="Val Physics Loss", color="orange")
    plt.xlabel("Epoch", fontsize=18)
    plt.ylabel("Loss", fontsize=18)
    plt.title("Validation Data and Physics Losses", fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.savefig(
        f"{save_dir_figs}/val_pinn_losses.eps", bbox_inches="tight", format="eps"
    )
    plt.show()

    plt.figure(figsize=(10, 5))
    loglog(train_data_losses, label="Train Data Loss (MSE)", color="blue")
    loglog(train_phys_losses, label="Train Physics Loss", color="orange")
    loglog(val_data_losses, label="Val Data Loss (MSE)", color="blue", linestyle="--")
    loglog(val_phys_losses, label="Val Physics Loss", color="orange", linestyle="--")
    plt.xlabel("Epoch", fontsize=18)
    plt.ylabel("Loss", fontsize=18)
    plt.title("Data and Physics Losses (LogLog)", fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.savefig(
        f"{save_dir_figs}/pinn_losses_loglog.eps", bbox_inches="tight", format="eps"
    )
    plt.show()

    print(f"PINN loss plots saved to {save_dir_figs}")
...
def plot_benchmark_accumulated_datasets(benchmark_results):
    ...
    all_dataset_chunks_sizes = list(
        benchmark_results["accumulated_datasets"]["results_val_mae"].keys()
    )
    ...
    plt.grid(axis="y")
    plt.savefig(
        f"{SAVE_DIR_BENCHMARKS}/plot_benchmark_accumulated_training_chunk_sizes.eps",
        bbox_inches="tight",
        format="eps",
    )
    plt.show()
    ...
    plt.minorticks_off()
    plt.savefig(
        f"{SAVE_DIR_BENCHMARKS}/plot_bechmark_accumuluated_datasets_Mixed-data.eps",
        bbox_inches="tight",
        format="eps",
    )
    plt.show()
...
def plot_accuracy_per_fold(benchmark_results):
    ...
    fold_numbers = list(
        benchmark_results["cross_validation"]["results_val_mae_unscaled"].keys()
    )
    accuracies = list(
        benchmark_results["cross_validation"]["results_val_mae_unscaled"].values()
    )
    accuracies = np.array(accuracies) * 1e6
    ...
    plt.savefig(
        f"{SAVE_DIR_BENCHMARKS}/cross_validation_accuracy_folds.eps",
        bbox_inches="tight",
        format="eps",
    )
...
def plot_cv_indices(cv, X, y, train_inputs_shape, n_splits, lw=10):
    ...
    plt.savefig(
        f"{SAVE_DIR_BENCHMARKS}/plot_cross_validation_train_test_folds.eps",
        bbox_inches="tight",
        format="eps",
    )
    plt.show()
